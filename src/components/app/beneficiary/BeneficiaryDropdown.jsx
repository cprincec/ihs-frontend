...
// export default function BeneficiaryDropdown({beneficiaryDetails}) {
...
// 						<Menu.Item>
// 							{({active}) => (
// 								<Link to={`/beneficiaries/updatebeneficiary/${beneficiaryDetails?.id}`}
...
// 						<Menu.Item>
// 							{({active}) => (
// 								<Link to={`/beneficiaries/updatebeneficiary/${beneficiaryDetails?.id}/addhealthcoverage`}
// 											className={classNames(
// 												active ? 'bg-gray-100 text-gray-900' : 'text-gray-700',
// 												'block px-4 py-2 text-sm'
// 											)}
...
export default function BeneficiaryDropdown({ beneficiary }) {
  return (
    <Menu as="div" className="relative inline-block text-left pr-4">
      <div>
        ...
        <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
            <Menu.Item>
              {({ active }) => (
                <Link
                  to={`/beneficiaries/updatebeneficiary/${beneficiary?.id}`}
                  className={classNames(
                    active ? "bg-gray-100 text-gray-900" : "text-gray-700",
                    "block px-4 py-2 text-sm"
                  )}
            ...
            <Menu.Item>
              {({ active }) => (
                <Link
                  to={`/beneficiaries/updatebeneficiary/${beneficiary?.id}/addhealthcoverage`}
                  className={classNames(
                    active ? "bg-gray-100 text-gray-900" : "text-gray-700",
                    "block px-4 py-2 text-sm"
                  )}
